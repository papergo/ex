...
__IO uint16_t PWM_Duty=20;  // 占空比：PWM_Duty/L298N_TIM_PERIOD*100%
                             // 占空比为：50%时电机不转
...
//				
//				HAL_TIM_PWM_Start(&htimx_L298N,TIM_CHANNEL_1); //启动通道1
//        HAL_TIMEx_PWMN_Start(&htimx_L298N,TIM_CHANNEL_1);//启动互补通道1
//      }
//      else
//      {
//        HAL_TIM_PWM_Stop(&htimx_L298N,TIM_CHANNEL_1);
...
void L298N_DCMOTOR_Contrl(uint8_t number,uint8_t length,uint16_t speed)
{
  switch (number)
  {
    case 1:
      if(length==1)
      {
    __HAL_TIM_SET_COMPARE(&htimx_L298N,TIM_CHANNEL_1,200); //100 正转
    HAL_Delay(4000);
    __HAL_TIM_SET_COMPARE(&htimx_L298N,TIM_CHANNEL_1,450); //405=900/2，==停止
    HAL_Delay(100);
    ...
    __HAL_TIM_SET_COMPARE(&htimx_L298N,TIM_CHANNEL_1,450); //405=900/2，==停止
    HAL_Delay(100);
		__HAL_TIM_SET_COMPARE(&htimx_L298N,TIM_CHANNEL_1,0); //100 正转
    HAL_Delay(4000);
    __HAL_TIM_SET_COMPARE(&htimx_L298N,TIM_CHANNEL_1,450); //405=900/2，==停止
    HAL_Delay(100);
				HAL_TIMEx_PWMN_Stop(&htimx_L298N,TIM_CHANNEL_1);
      ...
      {
    __HAL_TIM_SET_COMPARE(&htimx_L298N,TIM_CHANNEL_1,100); //100 正转
    HAL_Delay(4000);
    __HAL_TIM_SET_COMPARE(&htimx_L298N,TIM_CHANNEL_1,450); //405=900/2，==停止
    HAL_Delay(100);
		__HAL_TIM_SET_COMPARE(&htimx_L298N,TIM_CHANNEL_1,100); //100 正转
